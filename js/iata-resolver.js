...
(function (root, factory) {
  ...
  function generateAviasalesFlightUrl(originCity, destCity, departDate, returnDate, adults = 1) {
    ...
    const returnDDMM = (returnDate && String(returnDate).trim() && String(returnDate) !== 'null' && String(returnDate) !== 'undefined') ? formatDDMM(returnDate, 7) : '';
    ...
    return `https://www.aviasales.com/search/${routeSegment}?marker=${encodeURIComponent(marker)}`;
  }

  function buildAviasalesProposalUrl(originIATA, destIATA, departDate, returnDate, adults = 2) {
    return generateAviasalesFlightUrl(originIATA, destIATA, departDate, returnDate, adults);
  }
...
if (typeof window !== 'undefined' && typeof TripMuraIATA !== 'undefined') {
  window.generateAviasalesFlightUrl = TripMuraIATA.generateAviasalesFlightUrl;
}
